...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error fixing contract 40:', error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    // Just check the current status of contract 40
    const [contract] = await db
      ...
      .where(eq(familyContracts.id, 40))
      .limit(1);
    ...
    if (contract) {
      ...
      return NextResponse.json({ error: 'Contract 40 not found' }, { status: 404 });
    }

  } catch (error) {
    console.error('Error checking contract 40:', error);
    return NextResponse.json(
