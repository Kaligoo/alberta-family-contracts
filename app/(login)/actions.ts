...
const saveFamilyContractSchema = z.object({
  userFullName: z.string().min(1, 'Your name is required'),
  partnerFullName: z.string().min(1, 'Partner name is required'),
  userJobTitle: z.string().optional(),
  partnerJobTitle: z.string().optional(),
  userIncome: z.string().optional(),
  partnerIncome: z.string().optional(),
  children: z.string().optional(), // JSON string
});

export const saveFamilyContract = validatedActionWithUser(
  saveFamilyContractSchema,
  async (data, _, user) => {
    try {
      const children = data.children ? JSON.parse(data.children) : [];
      
      const contractData: NewFamilyContract = {
        userId: user.id,
        userFullName: data.userFullName,
        partnerFullName: data.partnerFullName,
        userJobTitle: data.userJobTitle || null,
        partnerJobTitle: data.partnerJobTitle || null,
        userIncome: data.userIncome ? data.userIncome : null,
        partnerIncome: data.partnerIncome ? data.partnerIncome : null,
        children: children,
        status: 'draft',
      ...
      await logActivity(user.id, ActivityType.CREATE_CONTRACT);

      return { success: 'Family contract saved successfully!' };
    } catch (error) {
      console.error('Error saving family contract:', error);
      return { error: 'Failed to save family contract. Please try again.' };
    }
  }
);
