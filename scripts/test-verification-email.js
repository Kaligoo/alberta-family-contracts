...
// Test the actual email verification template
// Usage: node scripts/test-verification-email.js <recipient-email>

require('dotenv').config({ path: '.env.development.local' });
// Import and test the verification email functionality
...
async function sendVerificationEmail(email, token) {
  ...
  return resend.emails.send({
    from: process.env.RESEND_FROM_EMAIL || 'Agreeable <emailverification@agreeable.ca>',
    to: [email],
...
async function testVerificationEmail(to) {
  try {
    console.log('Testing verification email template...');
    console.log(`Recipient: ${to}`);
    console.log(`From: ${process.env.RESEND_FROM_EMAIL || 'emailverification@agreeable.ca'}`);
    
    const result = await sendVerificationEmail(to, 'test-verification-token-123');
...
if (!recipient) {
  console.log('Usage: node scripts/test-verification-email.js <recipient-email>');
  console.log('Example: node scripts/test-verification-email.js ghorvath@agreeable.ca');
  process.exit(1);
}
...
testVerificationEmail(recipient);